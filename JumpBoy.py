...
demon_mode = False
pending_world = None
pending_music_path = 'img/music.wav'
privacy_seen_this_launch = False
privacy_from_start = False
...
def show_privacy_notice(world_num=None, music_path='img/music.wav', from_start=False):
    global privacy_active, world_select, pending_world, pending_music_path, privacy_from_start
    privacy_active = True
    world_select = False
    pending_world = world_num
    pending_music_path = music_path
    privacy_from_start = from_start
...
while run == True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not main_menu and not world_select and not settings_active and not leaderboard_active and not privacy_active and game_over != 2:
    ...
    if leaderboard_active:
        ...
        #Privacy notice skrevet med hjelp av KI
        screen.fill((0, 50, 100))
        draw_text('PRIVACY NOTICE', font, light_blue, screen_width // 2 - 255, 50)
        ...
        section_y = 170
        line_height = 28
        privacy_heading_font = pygame.font.SysFont('Bauhaus 93', 26, bold=True)
        privacy_body_font = pygame.font.SysFont('Bauhaus 93', 24)

        left_sections = [
            ("What we store", white),
            ("Username and score data", gray),
            ("World, time and coins", gray),
            ("Perfect run status", gray),
            ("", gray),
            ("Why we store it", white),
            ("To show leaderboard ranking", gray),
            ("To compare runs over time", gray),
            ("", gray),
            ("What we dont store", white),
            ("No IP addresses", gray),
            ("No passwords or payment data", gray),
            ("No other sensitive personal data", gray),
            ("", gray),
        ]
        right_sections = [
            ("How we use it", white),
            ("Only inside JumpBoy", gray),
            ("No ads or tracking scripts", gray),
            ("No sharing with third parties", gray),
            ("", gray),
            ("Contact", white),
            ("Owner: Ebbe Zelow", gray),
            ("Email: 19zelebb@stud.akademiet.no", gray),
        ]

        y = section_y
        for line, color in left_sections:
            if line:
                if color == white:
                    draw_text(line, privacy_heading_font, white, left_x, y, outline_thickness=2)
                else:
                    draw_text(line, privacy_body_font, white, left_x, y, outline_thickness=0)
            y += line_height

        y = section_y
        for line, color in right_sections:
            if line:
                if color == white:
                    draw_text(line, privacy_heading_font, white, right_x, y, outline_thickness=2)
                else:
                    draw_text(line, privacy_body_font, white, right_x, y, outline_thickness=0)
            y += line_height

        continue_text_surface = font_score.render("Press OK to continue", True, white)
        continue_text_rect = continue_text_surface.get_rect(center=(screen_width // 2, 635))
        screen.blit(continue_text_surface, continue_text_rect)
        ok_rect = pygame.Rect(325, 665, 150, 65)
        pygame.draw.rect(screen, light_blue, ok_rect, border_radius=12)
        pygame.draw.rect(screen, white, ok_rect, 3, border_radius=12)
        ok_text_surface = font_score.render("OK", True, white)
        ok_text_rect = ok_text_surface.get_rect(center=ok_rect.center)
        screen.blit(ok_text_surface, ok_text_rect)

        if pygame.mouse.get_pressed()[0] == 1 and ok_rect.collidepoint(pygame.mouse.get_pos()):
            plop_fx.play()
            privacy_active = False
            privacy_seen_this_launch = True
            if pending_world is not None:
                start_game_session(pending_world, pending_music_path)
            elif privacy_from_start:
                main_menu = False
                world_select = True
        ...
        if start_button.draw(screen):
            ...
            if privacy_seen_this_launch:
                ...
                show_privacy_notice(from_start=True)
